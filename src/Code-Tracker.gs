...
function _fetchActivePackages() {
  var url = _fbUrl(FB_COLL_COLIS) + '&pageSize=200';
...
function _updateFirestore(docId, fields) {
  var url = _fbUrl(FB_COLL_COLIS + '/' + docId);
...
function setup() {
  ...
  props.setProperty('FIREBASE_PROJECT_ID', 'familyhub-colis-8abbd');
  props.setProperty('FIREBASE_API_KEY', 'REMPLACER_PAR_VOTRE_CLE');
