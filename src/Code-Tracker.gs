...
var FB_API_KEY             = '';
var FB_COLL                = 'colis';
var MAX_PER_RUN            = 10;   // Max colis scrappés par exécution
var SKIP_IF_UPDATED_WITHIN = 90;   // Ignorer si mis à jour il y a moins de 90 min
...
function _fetchActivePackages() {
  ...
  return docs.map(function(doc) {
    ...
    if (active.indexOf(p.status) < 0) return false;
    if (!p.trackingLink && !p.trackingNum) return false;
    // Ignorer les colis mis à jour récemment
    if (p.lastUpdated) {
      var age = (now - new Date(p.lastUpdated).getTime()) / 60000; // en minutes
      if (age < SKIP_IF_UPDATED_WITHIN) return false;
    }
    return true;
  }).slice(0, MAX_PER_RUN); // Limiter le nombre de colis par run
}
