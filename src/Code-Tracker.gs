 ...
 * FIX CRITIQUE : _updateFirestore() ajoute updateMask.fieldPaths.
 *   Sans ce paramètre, le PATCH Firestore REMPLACE le document entier
 *   par les seuls champs envoyés (status/lastUpdated/events) et
...
// FIX CRITIQUE : updateMask.fieldPaths = clés réellement envoyées.
// Sans ça, Firestore REMPLACE le document entier (perte carrier/trackingNum/...).
function _updateFirestore(docId, fields) {
  var url = _fbUrl(TRACKER_FB_COLL + '/' + docId);
  Object.keys(fields).forEach(function(k) {
    url += '&updateMask.fieldPaths=' + encodeURIComponent(k);
  });
...
function _writeLog(summary) {
  try {
    ...
    Object.keys(logFields).forEach(function(k) {
      url += '&updateMask.fieldPaths=' + encodeURIComponent(k);
    });
    var body = { fields: _toFields(logFields) };
